...
void compat_test_main()
{
    ...
    assert(
        memcmp(
            album_content->data,
            ID3v2_to_unicode("Smoke And Shadows"),
            ID3v2_strlent("Smoke And Shadows")
        ) == 0
    );
    ...
    assert(
        memcmp(
            artist_content->data,
            ID3v2_to_unicode("Ethereal Darkness"),
            ID3v2_strlent("Ethereal Darkness")
        ) == 0
    );

    ID3v2_frame* title_frame = tag_get_title(tag);
    ID3v2_frame_text_content* title_content = parse_text_frame_content(title_frame);
    assert(memcmp(title_content->data, ID3v2_to_unicode("Rivers"), ID3v2_strlent("Rivers")) == 0);
